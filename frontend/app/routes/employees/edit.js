...
export default Route.extend({
  model(param){
    return RSVP.hash({
      employee: this.get('store').findRecord('employee',param.id),
      picture: this.get('store').createRecord('picture'),
  ...
  setupController(controller, model) {
    ...
    model.reportToEmployees.forEach(function (reportToEmployee) {
      if(reportToEmployee.get('firstName')!=null){
        if(reportToEmployee.get('firstName')!=model.employee.get('firstName')){
